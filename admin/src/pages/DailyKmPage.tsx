                                    ...
                                    d.startMissingList.map((row) => (
                                      <li key={row.vehicleId} className="flex items-start justify-between gap-2 px-3 py-2 text-xs">
                                        ...
                                        <span className="min-w-0 flex-1 text-right">
                                          <span className="block truncate text-slate-600 dark:text-slate-300">{row.driverName}</span>
                                          {row.driverPhone ? (
                  ...
                  <Fragment key={r.id}>
                    <tr className="app-table-row">
                      ...
                      <td rowSpan={2} className="p-3 align-top">
                        <div className="min-w-0">
                          <div className="truncate">{r.driver.fullName}</div>
                          {r.driver.phone ? (
                            <div className="mt-0.5 truncate text-xs font-medium tabular-nums text-slate-500 dark:text-slate-400">
                              {r.driver.phone}
                            </div>
                          ) : null}
                        </div>
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-3 sm:p-6">
          ...
          <div className="relative z-[81] w-full max-w-3xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900">
            ...
            <div className="h-[360px] w-full">
              <MapContainer center={mapCenter} zoom={14} maxZoom={LEAFLET_MAP_MAX_ZOOM} className="h-full w-full" scrollWheelZoom>
                <MapBaseLayers />
