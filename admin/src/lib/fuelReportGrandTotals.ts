...
export type FuelReportGrandTotals = {
  ...
  totalSystemAmount: number | null;
  totalVendorAmount: number | null;
  totalAmountDiff: number | null;
...
function sumAmounts(arr: (number | null)[]): number | null {
  let s = 0;
  let any = false;
  for (const x of arr) {
    if (x != null && Number.isFinite(x)) {
...
function addAmountToDay(
  bucket: (number | null)[],
  dayIndex: number,
  value: number | null,
) {
  if (value == null || !Number.isFinite(value)) return;
  bucket[dayIndex] = Math.round((bucket[dayIndex] ?? 0) + value);
}

/** Барча машиналар бўйича кунлик ва умумий жами (m³ ва сумма). */
export function computeFuelReportGrandTotals(
  ...
  const systemAmountByDay: (number | null)[] = Array.from(
    { length: daysInMonth },
    () => null,
  );
  const vendorAmountByDay: (number | null)[] = Array.from(
    { length: daysInMonth },
    () => null,
  );
  ...
  for (const v of vehicles) {
    ...
    for (let i = 0; i < daysInMonth; i += 1) {
      ...
      addAmountToDay(systemAmountByDay, i, v.systemAmountByDay?.[i] ?? null);
      addAmountToDay(vendorAmountByDay, i, v.vendorAmountByDay?.[i] ?? null);
    }
  }
  ...
  const totalSystemAmount = sumAmounts(systemAmountByDay);
  const totalVendorAmount = anyVendorEntered ? sumAmounts(vendorAmountByDay) : null;
  const totalAmountDiff =
    anyVendorEntered && (totalSystemAmount != null || totalVendorAmount != null)
      ? (totalSystemAmount ?? 0) - (totalVendorAmount ?? 0)
      : null;
